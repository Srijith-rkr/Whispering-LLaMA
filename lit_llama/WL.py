...
# We implement only LLAMA, Block and MHA , you import MLP and RMSNorm from the main model 
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            # Learnable matrix for the adapter implemented as embedding layer - equivalent to ML in the paper
            ...
            # Adapter following auto-encoder mechanism to realign Whisper Key Tensors + Proposed initilization mechanism
            self.projection_rms_key = RMSNorm(1280)
            self.projection_key_matrix_down = torch.nn.Parameter(torch.zeros([1280,1280//8]).fill_diagonal_(1))
            self.projection_key_matrix_up =  torch.nn.Parameter(torch.zeros([1280//8,1280]).fill_diagonal_(1))

            # Adapter following auto-encoder mechanism to realign Whisper Value Tensors + Proposed initilization mechanism
            self.projection_rms_value = RMSNorm(1280)
            self.projection_value_matrix_down = torch.nn.Parameter(torch.zeros([1280,1280//8]).fill_diagonal_(1))
            self.projection_value_matrix_up =  torch.nn.Parameter(torch.zeros([1280//8,1280]).fill_diagonal_(1))

            # To add trainable parameters towards adjusting the LLaMA query tensors
            self.projection_query_matrix_128to128 = torch.nn.Parameter(torch.zeros([128,128]).fill_diagonal_(1))
            self.projection_query_matrix_32to32 =   torch.nn.Parameter(torch.zeros([32,32]).fill_diagonal_(1))

            self.projection_gating_factor = torch.nn.Parameter(torch.ones(1))

            # To be initilised with weights from the Whisper model
    ...
    def forward(
        ...
        # "q","k","v" denotes LLaMA's embeddings and "key" and "value" denote whisper's embeddings
        ...
        # swapping head out of the matrix multiplication. To get the shape batch, heads, context, latent-vector
        ...
        if self.block_idx >= self.adapter_start_layer:
            ...
            if w_kv_cache is not None:
                ...
                #Adapter AW forward pass
                key = self.whisper_key(audio_features)
                key = self.projection_rms_key(key) 
                ...
                value = value @ self.projection_value_matrix_down
                value = F.silu(value)
                value = value @ self.projection_value_matrix_up

                value = value.view(B,20,1500,64)

                # Padding Mechanism explained in the paper
                ...
                for num in range(B):
                    padded_keys[num] = torch.zeros([1500,128]).fill_diagonal_(1).repeat(32, 1, 1)
                    padded_keys[num,:20,:,:64] = key[num]

                    padded_values[num] = torch.zeros([1500,128]).fill_diagonal_(1).repeat(32, 1, 1)
