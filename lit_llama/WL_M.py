...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            self.projection_key_matrix_down = torch.nn.Parameter(torch.zeros([1280,1280//16]).fill_diagonal_(1))
            self.projection_key_matrix_up =  torch.nn.Parameter(torch.zeros([1280//16,1280]).fill_diagonal_(1))
