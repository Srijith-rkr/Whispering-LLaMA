...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            self.projection_key_matrix_down = torch.nn.Parameter(torch.zeros([1280,1280//32]).fill_diagonal_(1))
            self.projection_key_matrix_up =  torch.nn.Parameter(torch.zeros([1280//32,1280]).fill_diagonal_(1))
            ...
            # We reuse the same Adapter for both Key and Value in case of WL-S and WL-M. WL has two Adapters, one for Key and one for Value
