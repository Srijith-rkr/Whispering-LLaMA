...
log_interval = 1
run_name = f'ablation_Masked_{dataset}_MINIHP_{learning_rate}_randn_audio_features'
...
def train(
    ...
    for iter_num in range(max_iters):
        ...
        temp = torch.randn(audio_features.shape,device = audio_features.device,dtype=audio_features.dtype)
        logits = model(input_ids, audio_features = temp)
