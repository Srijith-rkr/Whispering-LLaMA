...
from lit_llama.big_adapter import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
# To run inference on one Adapter Checkpoint
    ...
    for datapoint in tqdm.tqdm(data):
        ...
        y = generate(model =model, idx = encoded, max_new_tokens =150,max_seq_length=2048,temperature=0.2, top_k=1, eos_id=tokenizer.eos_id)

        model.reset_cache()
        output = tokenizer.decode(y)
        print(f'Prediction  {output[len(tokenizer.decode(encoded)):]}')
