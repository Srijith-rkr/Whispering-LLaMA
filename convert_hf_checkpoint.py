...
def convert_hf_checkpoint(
    *,
    output_dir: Path = Path("/home/dagaa/Srijith/LLaMA_7B/Alpaca_PY"),
    checkpoint_dir: Path = Path("/home/dagaa/Srijith/LLaMA_7B/Alpaca"),
    model_size: str = "7B",
    dtype: str = "float32",
    verify: bool = False,
) -> None:
    ...
    for bin_file in bin_files:
        ...
        for name, param in hf_weights.items():
            ...
            if "model.layers" in name:
                ...
                if ('lm_head.weight' in name) or ('model.embed_tokens.weight' in name):  # added tis exta condition becuase the alpaca weights from stanford alpaca has an extra dimension
                    sd[weight_map[name]].copy_(param[:32000,:])
                else:
